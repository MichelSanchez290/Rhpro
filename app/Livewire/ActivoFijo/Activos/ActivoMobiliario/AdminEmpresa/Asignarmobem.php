...
class Asignarmobem extends Component
{
    use WithFileUploads;
    ...
    {
        if ($sucursalId) {
            $this->activosFiltrados = ActivoMobiliario::where('sucursal_id', $sucursalId)
                ->where('status', 'Activo')
                ->get();
    ...
    public function asignarActivo()
    {
        ...
        $rutaBase = 'ActivoFijo/Activos/ActivoMobiliario/Asignaciones/AdminEmpresa';
        ...
        session()->flash('success', 'Activo tecnológico asignado correctamente.');
        return redirect()->route('mostrarasigntecem');
    }

    public function render()
    {
        return view('livewire.activo-fijo.activos.activo-mobiliario.admin-empresa.asignarmobem')->layout('layouts.navactivos');
    }
}
