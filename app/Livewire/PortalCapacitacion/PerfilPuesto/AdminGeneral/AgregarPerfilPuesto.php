...
class AgregarPerfilPuesto extends Component
{
    ...
    public function mount(){
        ...
        $this->empresas = Empresa::all(); // Obtener todas las empresas
        $this->sucursales = []; // Inicializar vacío
        $this->funcionesDisponibles = []; // Inicializar vacío
        $this->internasDisponibles = []; // Inicializar vacío
        $this->externasDisponibles = []; // Inicializar vacío
        $this->responsabilidadesDisponibles = []; // Inicializar vacío
        $this->humanasDisponibles = []; // Inicializar vacío
        $this->tecnicasDisponibles = []; // Inicializar vacío
    }

    public function updatedEmpresaId()
    {
        // Obtener sucursales relacionadas con la empresa seleccionada
        $this->sucursales = Empresa::find($this->empresa_id)?->sucursales ?? [];
        // Resetear sucursal y funciones
        $this->sucursal_id = null;
        $this->funcionesDisponibles = [];
        $this->internasDisponibles = [];
        $this->externasDisponibles = [];
        $this->responsabilidadesDisponibles = []; 
        $this->humanasDisponibles = [];
        $this->tecnicasDisponibles = [];
    }

    public function updatedSucursalId()
    {
        // Filtrar funciones por empresa y sucursal seleccionada
        $this->funcionesDisponibles = FuncionEspecifica::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();

        $this->internasDisponibles = RelacionInterna::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();
        
        $this->externasDisponibles = RelacionExterna::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();

        $this->responsabilidadesDisponibles = ResponsabilidadUniversal::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();

        $this->humanasDisponibles = FormacionHabilidadHumana::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();

        $this->tecnicasDisponibles = FormacionHabilidadTecnica::where('empresa_id', $this->empresa_id)
            ->where('sucursal_id', $this->sucursal_id)
            ->get();
    }
    ...
    public function savePerfil()
{
    ...
    {
        return view('livewire.portal-capacitacion.perfil-puesto.admin-general.agregar-perfil-puesto')->layout("layouts.portal_capacitacion");
    }
}
