<div>
    ...
    <div class="bg-gradient-to-r from-[#1763A6] to-[#1EA4D9] text-white font-bold text-xl py-4 px-6 rounded-t-lg shadow-lg flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
            <!-- Botón Agregar -->
            <button onclick="window.location.href='{{ route('agregartec') }}'"
    ...
    <div class="bg-white rounded-b-lg shadow-2xl p-6 mt-2">
        <livewire:activo-fijo.tablas-power.tecnologia-table />
    </div>
</div>
@push('scripts')
    <script>
        // Inicializar los filtros de PowerGrid
        document.addEventListener('livewire:load', function () {
            Livewire.emit('pg:enableFilters');
        });
    </script>
@endpush
