    ...
    <div class="h-screen overflow-y-auto">
        <div class="my-5">
            <div class="bg-gradient-to-r from-[#1763A6] to-[#1EA4D9] text-white font-bold text-xl py-4 px-6 rounded-t-lg shadow-lg">
                <h1 class="text-center text-2xl sm:text-3xl font-bold text-white">Asignar Activo Souvenir</h1>
            </div>
            <div class="bg-white rounded-b-lg p-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="my-2">
                        <div class="py-2 my-2">
                            <label class="text-gray-700 font-bold text-xl">Sucursal</label>
                        </div>
                        <input type="text" class="block w-full border-2 px-2 py-2 text-sm sm:text-md rounded-md my-2 text-black" 
                               value="{{ $sucursal->nombre_sucursal }}" readonly>
                    </div>

                    <!-- Select de Activo Tecnológico -->
                    <div class="my-2">
                        <div class="py-2 my-2">
                            <label for="activo" class="text-gray-700 font-bold text-xl">Activo Souvenir</label>
                        </div>
                        <select id="activo" wire:model.live="activoSeleccionado" class="block w-full border-2 px-2 py-2 text-sm sm:text-md rounded-md my-2 text-black">
                            <option value="">Seleccione un activo</option>
                            @foreach ($activosFiltrados as $activo)
                                <option value="{{ $activo->id }}">{{ $activo->productos }}</option>
                            @endforeach
                        </select>
