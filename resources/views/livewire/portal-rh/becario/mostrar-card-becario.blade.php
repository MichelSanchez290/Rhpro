<div class="h-full bg-gray-200 p-8">
    ...
    <div class="flex relative rounded-md w-full max-w-3xl mx-auto">
        <input type="text" wire:model.live="search" placeholder="Nombre de usuario o Clave de becario"
            class="w-full p-3 rounded-md border-2 border-r-white rounded-r-none
             border-gray-300 placeholder-gray-500" />
        <button class="inline-flex items-center gap-2 bg-blue-700 text-white text-lg font-semibold py-3 px-6 rounded-r-md">
                ...
                <div class="w-full mx-auto bg-white shadow-xl rounded-lg text-gray-900 p-4">
                    ...
                    <div class="mx-auto w-32 h-32 relative -mt-16 border-4 border-white rounded-full overflow-hidden">
                        <img class="object-cover object-center h-32 w-32"
                            src="{{ $usuarios[$becario->user_id]->profile_photo_path ? asset('storage/' . $usuarios[$becario->user_id]->profile_photo_path) : asset('img/user.png') }}"
                            alt="Foto de perfil">
                    </div>
                    ...
                    <div class="text-center mt-2">
                        ...
                        <p class="text-gray-500">
                            Reg Patronal:
                            {{ $registros_patronales[$becario->registro_patronal_id]->registro_patronal ?? 'Sin Departamento' }}
                        </p>
                        <p class="text-gray-500">
                            Empresa: {{ $emp[$usuarios[$becario->user_id]->empresa_id]->nombre ?? 'Sin Empresa' }}
                        </p>
                        <p class="text-gray-500">
                            Sucursal:
                            {{ $suc[$usuarios[$becario->user_id]->sucursal_id]->nombre_sucursal ?? 'Sin Sucursal' }}
                        </p>
                        <p class="text-gray-500">
                            Departamento:
                            {{ $depa[$usuarios[$becario->user_id]->departamento_id]->nombre_departamento ?? 'Sin Departamento' }}
                        </p>
                        <p class="text-gray-500">
                            Puesto: {{ $puest[$usuarios[$becario->user_id]->puesto_id]->nombre_puesto ?? 'Sin Puesto' }}
                        </p>
                    </div>
