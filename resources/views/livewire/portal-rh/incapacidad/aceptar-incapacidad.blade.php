    ...
    <div class="flex min-h-screen items-center justify-center py-3">
        <div class="grid bg-white rounded-lg shadow-xl w-full">
                    ...
                    @if (session()->has('message'))
                        <div x-data="{ show: true }" x-show="show"
                            ...
                            <div class="flex-1">
                                ...
                                @elseif(session('message') == 'Incapacidad aprobada y registrada.')
                                    <p class="text-sm">La solicitud se aprobó y se agrego al historial.</p>
                                @elseif(session('message') == 'Incapacidad rechazada.')
                                    <p class="text-sm">La solicitud fue rechazada y agregada al historial.</p>
                                @endif
                            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
                @foreach ($incapacidades_pendientes as $incapacidad)
                    <div class="w-full mx-auto bg-white shadow-xl rounded-lg text-gray-900 p-4">
                        ...
                        <div class="text-center mt-2">
                            ...
                            <p><strong>Motivo de incapacidad:</strong> {{ $incapacidad->motivo }}</p>
                            <p><strong>Fecha inicio:</strong> {{ $incapacidad->fecha_inicio }}</p>
                            <p><strong>Fecha final:</strong> {{ $incapacidad->fecha_final }}</p> <br>

                            @if ($incapacidad->documento)
                                <a href="{{ asset('' . $incapacidad->documento) }}" target="_blank"
                                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded inline-block mt-2">
                                    Ver justificante
                                </a>
                            @else
                                <p class="text-red-500 font-bold">No se adjuntó documento.</p>
                            @endif
                        </div>

                        <div class="flex gap-4 mt-4">
                            <button wire:click="aprobar({{ $incapacidad->id }})"
                                class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                Aprobar
                            </button>

                            <button wire:click="rechazar({{ $incapacidad->id }})"
