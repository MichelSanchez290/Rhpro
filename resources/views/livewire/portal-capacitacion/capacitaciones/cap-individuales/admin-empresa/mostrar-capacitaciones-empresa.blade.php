<div class="max-w-4xl px-10 my-6 py-8 bg-white rounded-lg shadow-md mx-auto relative">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{route('vermasUsuariosEmpresa', Crypt::encrypt($userSeleccionado))}}"
    ...
    <div class="flex space-x-4 border-b border-gray-300 mb-6">
        <a href="{{ route('verCapacitacionesIndEmpresa', Crypt::encrypt($userSeleccionado)) }}"
            class="px-6 py-3 border-b-2 transition-all duration-300 
        {{ $rutaActual === 'verCapacitacionesIndEmpresa' ? 'border-blue-500 text-blue-600 font-semibold' : 'border-transparent text-gray-600 hover:text-blue-500' }}">
            📌 Individuales
        </a>

        <a href="{{ route('verCapacitacionesGrupalesAdminEmpresa', Crypt::encrypt($userSeleccionado)) }}"
            class="px-6 py-3 border-b-2 transition-all duration-300 
        {{ $rutaActual === 'verCapacitacionesGrupalesAdminEmpresa' ? 'border-blue-500 text-blue-600 font-semibold' : 'border-transparent text-gray-600 hover:text-blue-500' }}">
            👥 Grupales
        </a>
    </div>
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        <button onclick="window.location.href='{{ route('agregarCapacitacionesIndEmpresa', Crypt::encrypt($userSeleccionado)) }}'" 
        ...
        @foreach ($capacitaciones as $capacitacion)
            <div class="mt-8 p-4 border border-gray-300 rounded-lg shadow-md hover:shadow-lg transition-all relative">
                ...
                <div class="mt-2">
                    ...
                    <p class="text-gray-700 font-semibold">Ocupación especifica: {{ $capacitacion->ocupacion_especifica }}</p>
                </div>
