<div class="relative max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-6 border border-gray-200">
    <button class="absolute top-3 right-3 bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-3 rounded-full"
        wire:click="cerrar()">
    ...
    <p class="text-gray-600"><strong>Puestos que le reportan:</strong>
        {{ $puestoSeleccionado->puestos_que_le_reportan }}</p>
    ...
    <p class="text-gray-600"><strong>Rango toma de desiciones:</strong> {{ $puestoSeleccionado->rango_toma_desicones }}
    </p>
    ...
    <p class="text-gray-600"><strong>Estado civil deseable:</strong> {{ $puestoSeleccionado->estado_civil_deseable }}
    </p>
    ...
    <p class="text-gray-600"><strong>Experiencia Requerida:</strong> {{ $puestoSeleccionado->experiencia_requerida }}
    </p>
                ...
                <table class="w-full bg-white border-collapse">
                    <thead>
                        ...
                        <tr class="bg-blue-50">
                            ...
                            @for ($i = 1; $i <= 5; $i++)
                                <th class="px-3 py-2 text-center text-xs text-gray-600 font-medium">{{ $i }}
                                </th>
                            @endfor
                        </tr>
                    </thead>
    ...
    <div class="mt-8 bg-white rounded-lg shadow-sm p-6 border border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-400">
                ...
                <div class="mt-2">
                    <p class="text-lg font-medium text-gray-800">
                        {{ $puestoSeleccionado?->elaboro_nombre ?? 'No definido' }}
                    </p>
                    <p class="text-sm text-gray-500">
                        {{ $puestoSeleccionado?->elaboro_puesto ?? 'No definido' }}
                    </p>
                </div>
            </div>
            ...
            <div class="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-400">
                ...
                <div class="mt-2">
                    <p class="text-lg font-medium text-gray-800">
                        {{ $puestoSeleccionado?->reviso_nombre ?? 'No definido' }}
                    </p>
                    <p class="text-sm text-gray-500">
                        {{ $puestoSeleccionado?->reviso_puesto ?? 'No definido' }}
                    </p>
                </div>
            </div>
            ...
            <div class="bg-green-50 p-4 rounded-lg border-l-4 border-green-400">
                ...
                <div class="mt-2">
                    <p class="text-lg font-medium text-gray-800">
                        {{ $puestoSeleccionado?->autorizo_nombre ?? 'No definido' }}
                    </p>
                    <p class="text-sm text-gray-500">
                        {{ $puestoSeleccionado?->autorizo_puesto ?? 'No definido' }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 flex items-center justify-between bg-gray-50 px-4 py-3 rounded-lg">
            ...
            @php
                $statusColor = match ($puestoSeleccionado?->status) {
            ...
            <span class="px-3 py-1 rounded-full text-sm font-semibold {{ $statusColor }}">
                {{ $puestoSeleccionado?->status ?? 'No definido' }}
            </span>
        </div>
    </div>
