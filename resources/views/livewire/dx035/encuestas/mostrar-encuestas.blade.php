                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <div class="flex items-center">
                                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                                        <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ $encuesta->EncuestasContestadas }}%"></div>
                                    </div>
                                    <span class="ml-2 text-sm font-medium text-gray-700">{{ $encuesta->EncuestasContestadas }}%</span>
                                </div>
                            </td>
...
<script>
    ...
    function copiarClave(clave) {
        if (navigator.clipboard) {
            // Usar navigator.clipboard si sirve
            navigator.clipboard.writeText(clave)
                .then(() => alert("Clave copiada al portapapeles: " + clave))
                .catch(() => fallbackCopyText(clave));
        } else {
            // metodo alternativo si navigator.clipboard no sirve
    ...
    function compartirEnlace(enlace) {
        if (navigator.clipboard) {
            ...
            //eétodo alternativo si navigator.clipboard no sirve
